...
1. **Unit tests**: Mock `IEmbeddingService` and `QdrantClient`; test `QdrantNuGetReadStore` query construction
2. **Integration test**: Stand up a local Qdrant instance (Docker), index a small NuGet package, run `SearchDocumentsAsync` and assert results come back
3. **End-to-end**: Run the indexer against a test source, then start the server, connect via MCP client, call `query_docs` and `resolve_library`, compare result quality to SQLite baseline
4. **Idempotency**: Index the same source twice, assert point counts don't grow
5. **Version selection**: Verify `ListVersionsAsync` returns correct ordering and `is_prerelease` / `is_deprecated` flags are respected via payload filters
